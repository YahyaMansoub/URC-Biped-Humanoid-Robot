...
// ===================== Bluetooth =====================
// BT TX -> Arduino D2 (RX)
// BT RX -> Arduino D3 (TX)  (if you only receive, you can leave TX unused)
...
#define stepClearance  2.2f   // keep your tuned value
...
volatile bool doOneStep = false;     // set by BT 'F'
...
// Initial pose (exact known-good counts)
void setPose() {
...
// ===================== One full step cycle (then return to pose) =====================
void takeOneStep(float stepLength, int stepVelocity) {
  // Phase 1: RIGHT stance, LEFT swing
  for (float i = stepLength; i >= -stepLength; i -= 0.25f) {
    float xR = i * STANCE_R_SCALE;
    float xL = i * SWING_L_SCALE;
  ...
  // IMPORTANT: after each command-step, always return to the initial pose
  setPose();
  delay(200);
}
...
// ===================== Setup helpers =====================
void initialize() {
  // You asked: “always take the initial position”
  // So we just go to pose and hold it.
  setPose();
  delay(300);
}
...
void pollBT() {
  while (BT.available()) {
    ...
    if (c == 'F') {
      ...
      // optional: speed digit (0 slow -> 70ms, 9 fast -> 16ms)
      int v = c - '0';
      stepVelocity = 70 - (v * 6);
...
void setup() {
  ...
  Serial.println("BT ready: send F(one step), P(pose), D(debug), 0..9(speed)");
  initialize();
}
...
void loop() {
  ...
  if (doOneStep) {
    doOneStep = false;               // consume command
    takeOneStep(stepLength, stepVelocity);
  } else {
    // always keep robot in initial pose while idle
    setPose();
    delay(20);
  }
}
