...
// ===================== Bluetooth =====================
// BT TX -> Arduino D2 (RX)
// (Optional) BT RX -> Arduino D3 (TX) via divider if you want to send back
...
// ===================== Multi-cycle per click =====================
#define CYCLES_PER_CLICK 3
volatile int pendingCycles = 0; // each cycle = one full gait cycle
...
void takeOneStep(float stepLength, int stepVelocity) {
  ...
  // After each cycle, return to initial pose
  setPose();
  delay(200);
}
...
void pollBT() {
  while (BT.available()) {
    ...
    if (c == 'F') {
      pendingCycles += CYCLES_PER_CLICK;   // <<<<<< 3 cycles per click
      if (pendingCycles > 30) pendingCycles = 30; // cap
    } else if (c == 'P') {
      pendingCycles = 0;
      setPose();
    } else if (c == 'D') {
...
void setup() {
  ...
  Serial.println("BT ready: F=3 cycles, P=pose, D=debug, 0..9=speed");
  initialize();
}
